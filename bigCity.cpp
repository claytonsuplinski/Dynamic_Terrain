...
BigCity::BigCity() : Object(){}

BigCity::~BigCity(){}

bool BigCity::Initialize()
{
	if (this->GLReturnedError("BigCity::Initialize - on entry"))
		return false;
...
char *chars = reinterpret_cast<char*>(&myInt);
	...
	if (this->GLReturnedError("BigCity::Initialize - on exit"))
		return false;

	return true;
}

void BigCity::TakeDown(){super::TakeDown();}

void BigCity::saveBuildingVertices(){
	ofstream outputFile;
...
void BigCity::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size, const float time){

	if (this->GLReturnedError("BigCity ::Draw - on entry")){
		return;	
	}
	...
	bool insideBigCity = (userPosition.x > 150 && userPosition.x < 6560 && userPosition.z > 150 && userPosition.z < 9054);
	...
	//		|| (abs(cityBlocksBuildingsPositions.at(i).x - userPosition.x) < 200 && abs(cityBlocksBuildingsPositions.at(i).z - userPosition.z) < 3000)){
		...
		if(absDistX < 3500 && absDistZ < 3500){
			...
			if(absDistX < 400 && absDistZ < 400
				...
				if(insideBigCity){ //inside the city
	...
	if (this->GLReturnedError("BigCity::Draw - on exit")){
		return;
	}
}


bool BigCity::buildingInFront(int buildingIndex, float distance, float angleOffset, float buffer){
	if(distance > 0){
...
float BigCity::sign(vec2 p1, vec2 p2, vec2 p3)
{
  return (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
}

bool BigCity::PointInTriangle(vec2 pt, vec2 v1, vec2 v2, vec2 v3)
{
  bool b1, b2, b3;
...
bool CCW2(vec2 C, vec2 W1, vec2 W2){
	vec2 ab = W1-C;
	vec2 ac = W2-C;
...
bool BigCity::linesIntersect(vec2 a1, vec2 a2, vec2 b1, vec2 b2){
	return (CCW2(a1, b1, b2) != CCW2(a2, b1, b2)) && (CCW2(b1, a1, a2) != CCW2(b2, a1, a2));
}


void BigCity::loadBuildings(){
